...
{
        int p;
        if(src > 0 && src < CV_PI/4)
             p=1;
        else if(src >= CV_PI/4 && src < CV_PI/2)
             p=2;
        else if(src >= CV_PI/2 && src < 3*CV_PI/4)
             p=4;
        else if(src >= 3*CV_PI/4 && src < CV_PI)
             p=8;
        else if(src >= CV_PI && src < 5*CV_PI/4)
             p=16;
        else if(src >= 5*CV_PI/4 && src < 3*CV_PI/2)
             p=32;
        else if(src >= 3*CV_PI/2 && src < 7*CV_PI/4)
             p=64;
        else if(src >= 7*CV_PI/4 && src < 2*CV_PI)
             p=128;
...
Mat  myOGBP(Mat& src)
{
     ...
     // Mat src = imread("1_1.pgm",0);

      GaussianBlur( src, src, Size(3,3), 0, 0, BORDER_DEFAULT );

      Mat grad_x, grad_y;
      Mat G1_0, G2_0, G1_pi2, G2_pi2, G1_pi, G2_pi, G1_3pi2, G2_3pi2;
      ...
      //0
      convertScaleAbs( grad_x, G1_0, 1, 0 );
      convertScaleAbs( grad_y, G2_0, 1, 0 );
      //pi/2
      convertScaleAbs( grad_y, G1_pi2, 1, 0 );
      convertScaleAbs( grad_x, G2_pi2, -1, 0 );
      //pi
      convertScaleAbs( grad_x, G1_pi, -1, 0 );
      convertScaleAbs( grad_y, G2_pi, -1, 0 );
      //3pi/2
      convertScaleAbs( grad_y, G1_3pi2, -1, 0 );
      convertScaleAbs( grad_x, G2_3pi2, 1, 0 );


      Mat dst1 = Mat(src.size(), CV_8U, Scalar(0));
      Mat dst2 = Mat(src.size(), CV_8U, Scalar(0));
      Mat dst3 = Mat(src.size(), CV_8U, Scalar(0));
      Mat dst4 = Mat(src.size(), CV_8U, Scalar(0));
      ...
      for(int i=0;i< src.rows;i++)
      {
           for(int j=0;j< src.cols;j++)
          {
               ...
               if(G1_0.at<uchar>(j,i)==0) G1_0.at<uchar>(j,i)=1;
               if(G2_0.at<uchar>(j,i)==0) G2_0.at<uchar>(j,i)=1;

               if(G1_pi2.at<uchar>(j,i)==0) G1_pi2.at<uchar>(j,i)=1;
               if(G2_pi2.at<uchar>(j,i)==0) G2_pi2.at<uchar>(j,i)=1;

               if(G1_pi.at<uchar>(j,i)==0) G1_pi.at<uchar>(j,i)=1;
               if(G2_pi.at<uchar>(j,i)==0) G2_pi.at<uchar>(j,i)=1;

               if(G1_3pi2.at<uchar>(j,i)==0) G1_3pi2.at<uchar>(j,i)=1;
               if(G2_3pi2.at<uchar>(j,i)==0) G2_3pi2.at<uchar>(j,i)=1;

               float a = atan2(G2_0.at<uchar>(j,i), G1_0.at<uchar>(j,i))+CV_PI;   //(0,2pi)
               float b = atan2(G2_pi2.at<uchar>(j,i), G1_pi2.at<uchar>(j,i))+CV_PI;   //(0,2pi)
               float c = atan2(G2_pi.at<uchar>(j,i), G1_pi.at<uchar>(j,i))+CV_PI;   //(0,2pi)
               float d = atan2(G2_3pi2.at<uchar>(j,i), G1_3pi2.at<uchar>(j,i))+CV_PI;   //(0,2pi)

               dst1.at<uchar>(j,i)= erzhi(a);
               dst2.at<uchar>(j,i)= erzhi(b);
               dst3.at<uchar>(j,i)= erzhi(c);
               dst4.at<uchar>(j,i)= erzhi(d);
      ...
      double aa,bb,cc,dd;
      minMaxLoc(dst1,0,&aa,0,0);
      minMaxLoc(dst2,0,&bb,0,0);
      minMaxLoc(dst3,0,&cc,0,0);
      minMaxLoc(dst4,0,&dd,0,0);
      ...
      Mat src2 = asRowMatrix(vec, CV_8U);
      //Mat src3;
      //transpose(src2,src3);
      //cout << "src3= " << src3.size() << endl;
...
class OGBP : public FaceRecognizer
{
private:
    int _num_components;
    double _threshold;
...
public:
    ...
    OGBP(int num_components = 0, double threshold = DBL_MAX) :
    ...
    // classification.
    OGBP(InputArrayOfArrays src, InputArray labels,
...
// Eigenfaces
//------------------------------------------------------------------------------
void OGBP::train(InputArrayOfArrays _src, InputArray _local_labels) {
    if(_src.total() == 0) {
    ...
    //cout << "src.total = "<< _src.total()<< endl;
    if(_src.total() > 1) {
   ...
   vector<Mat> getimg;
   _src.getMatVector(getimg);
  // cout << "getimg.size "<< getimg.size() << endl;
  // cout << "getimg[0].size "<< getimg[1].size() << endl;

   for(int i=0;i<n;i++)
   {

       Mat outimg = myOGBP(getimg[i]);
       //cout << "outimg size " << outimg.size() << endl;

       //Mat outimg2 = outimg.reshape(1,1);
       vec.push_back(outimg);
      // cout << "outimg size " << vec.size() << endl;

       //while(1);
   }

    Mat data1 = asRowMatrix(vec, CV_64FC1);
    //cout << "data1 size " << data1.size() << endl;
    //cout << "data1 row " << data1.rows << endl;
    //cout << "data1 cols " << data1.cols << endl;
    //while(1);
    ...
    // perform the PCA
    PCA pca(data1, Mat(), CV_PCA_DATA_AS_ROW, _num_components);
    cout << "doing PCA " << endl;
    ...
    // save projections
    for(int sampleIdx = 0; sampleIdx < data1.rows; sampleIdx++) {
        Mat p = subspaceProject(_eigenvectors, _mean, data1.row(sampleIdx));
        _projections.push_back(p);
    }
}

void OGBP::predict(InputArray _src, int &minClass, double &minDist) const {
    // get data
    Mat src = _src.getMat();
    Mat src1 = myOGBP(src);

    //cout << "src1.size = " << src1.size() << endl;
    //cout << "src1.total = " << src1.total()<< endl;
    //while(1);
    ...
    } else if(_eigenvectors.rows != static_cast<int>(src1.total())) {
        // check data alignment just for clearer exception messages
        string error_message = format("Wrong input image size. Reason: Training and Test images must be of equal size! Expected an image with %d elements, but got %d.", _eigenvectors.rows, src1.total());
        CV_Error(CV_StsBadArg, error_message);
    }
    ...
    Mat q = subspaceProject(_eigenvectors, _mean, src1.reshape(1,1));
    minDist = DBL_MAX;
    minClass = -1;
...
int OGBP::predict(InputArray _src) const {
    int label;
    double dummy;
...
void OGBP::load(const FileStorage& fs) {
    //read matrices
...
void OGBP::save(FileStorage& fs) const {
    // write matrices
    fs << "num_components" << _num_components;
    fs << "mean" << _mean;
    fs << "eigenvalues" << _eigenvalues;
...
{
    return new OGBP(num_components, threshold);
}
